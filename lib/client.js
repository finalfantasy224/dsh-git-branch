...
(function () {
  ...
  function makeGitBranchDock(react, h) {
    ...
    return function GitBranchDock(props) {
      ...
      }, [wsPath, cfgLoaded, appendLog, setBusy, setCfgName, setCfgEmail, setCfgLoaded]);
      ...
      var runCredential = useCallback(function () {
        ...
        if (host === '' || username === '' || password === '') {
          appendLog([{ time: nowTime(), repo: '-', message: '请填写 host / 用户名 / 密码', tone: 'fail' }]);
          return;
        }
        setBusy('credential');
        appendLog([{ time: nowTime(), repo: '-', message: '保存凭证 → ' + host, tone: 'info' }]);
        postJson('/git-config', { ws: wsPath, action: 'credential', host: host, username: username, password: password })
          .then(function (data) {
      ...
      }, [wsPath, busy, credHost, credUser, credPass, appendLog, setCredHost, setCredUser, setCredPass]);
      ...
      var tabConfig = h('div', { className: 'dshgb-form' },
        ...
        h('div', { className: 'dshgb-row' },
          h('span', { className: 'dshgb-form-label', style: { flex: 1 } }, '推送凭证（credential helper）'),
        ),
        h('div', { className: 'dshgb-hint' }, '凭证 helper 需先配置（如 git config --global credential.helper store）。下方保存的凭证写入该 helper。'),
        h('div', { className: 'dshgb-form-row' },
          h('input', {
            className: 'dshgb-input dshgb-input-full',
            placeholder: 'host，如 github.com',
            value: credHost,
            disabled: busyAny,
        ...
        }, busy === 'credential' ? spinnerIcon(h) : null, '保存凭证'),
      );
