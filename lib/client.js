...
(function () {
  ...
  var CSS = [
    ...
    '  flex: 1;',
    '}',
    '.dshgb-tab-content { flex: 1; min-height: 0; overflow-y: auto; }',
    '}',
    ...
    /* ---- workspace .env editor (config tab) ---- */
    '.dshgb-btn-small { padding: 1px 8px; font-size: 10px; }',
    '.dshgb-env-editor { display: flex; flex-direction: column; gap: 4px; }',
    '.dshgb-env-row { display: flex; gap: 4px; align-items: center; }',
    '.dshgb-env-key { width: 38%; flex: 0 0 auto; font-family: var(--dsw-alias-font-mono, ui-monospace, monospace); font-size: 10px; }',
    '.dshgb-env-value { flex: 1; font-family: var(--dsw-alias-font-mono, ui-monospace, monospace); font-size: 10px; }',
  ].join('\n');
  ...
  function injectCss() {
    ...
    if (tag !== null) {
      // HMR re-runs injectCss with the same tag; refresh the content so CSS
      // edits apply without a page refresh.
      tag.textContent = CSS;
      return;
    }
  ...
  function closeIcon(h) {
    return h('svg', { width: 11, height: 11, viewBox: '0 0 16 16', fill: 'none', 'aria-hidden': true },
  ...
  function makeGitBranchDock(react, h) {
    ...
    return function GitBranchDock(props) {
      ...
      // Where the credential host+username live: 'global' → all in the global
      // credential file (shared by every workspace); 'local' (default) →
      // host + username in the workspace .env, token still global.
      var credScopeInit = useState('local');
      var credScope = credScopeInit[0];
      var setCredScope = credScopeInit[1];
      var credInfoInit = useState(null); // { remoteHosts, envPath, hasToken, globalUsername, globalHasToken, globalHost }
      var credInfo = credInfoInit[0];
      var setCredInfo = credInfoInit[1];
      var envEntriesInit = useState(null); // null | [{key, value}]
      var envEntries = envEntriesInit[0];
      var setEnvEntries = envEntriesInit[1];
      ...
      // Keep the always-visible log box scrolled to the newest line.
      useEffect(function () {
      ...
      var loadConfig = useCallback(function () {
        ...
        Promise.all([
          ...
          postJson('/git-config', { ws: wsPath, action: 'read-ws-env' }),
        ]).then(function (values) {
          setCfgName(values[0].value || '');
          setCfgEmail(values[1].value || '');
          var wsCred = values[2];
          var globalCred = values[3];
          // Workspace-level defaults for the credential form. The scope radio
          // picks the source of host+username: 'local' → workspace .env (token
          // still global); 'global' → the global credential file entirely.
          var preferredHost = credScope === 'global'
            ? (globalCred.host || wsCred.host || '')
            : (wsCred.host || globalCred.host || '');
          setCredHost(preferredHost || (wsCred.remoteHosts && wsCred.remoteHosts[0]) || '');
          var preferredUser = credScope === 'global'
            ? (globalCred.username || wsCred.username || '')
            : (wsCred.username || globalCred.username || '');
          setCredUser(preferredUser);
          setCredPass('');
          setCredInfo({
            ...
            globalHost: globalCred.host || '',
            globalFile: globalCred.file || '',
          });
          // Full .env key/value list for the editor (null → not loaded yet).
          setEnvEntries(Array.isArray(values[4] && values[4].entries) ? values[4].entries : []);
          setCfgLoaded(true);
        }).catch(function () {
      ...
      }, [wsPath, cfgLoaded, credScope, appendLog, setBusy, setCfgName, setCfgEmail, setCfgLoaded, setCredHost, setCredUser, setCredPass, setCredInfo, setEnvEntries]);
      ...
      var runCredential = useCallback(function () {
        ...
        setBusy('credential');
        appendLog([{ time: nowTime(), repo: '-', message: '保存凭证 → ' + host + (credScope === 'global' ? '（全局，所有仓库共用）' : '（当前工作区）'), tone: 'info' }]);
        postJson('/git-config', {
          ws: wsPath,
          action: 'save-ws-cred',
          scope: credScope,
            ...
            if (data.ok) {
              ...
              setCredInfo(function (prev) {
                return prev === null ? prev : {
                  ...
                  globalHost: credScope === 'global' ? host : prev.globalHost,
      ...
      }, [wsPath, busy, credHost, credUser, credPass, credScope, appendLog, setCredHost, setCredUser, setCredPass, setCredInfo]);

      var updateEnvEntry = useCallback(function (index, value) {
        setEnvEntries(function (prev) {
          if (!prev) return prev;
          return prev.map(function (entry, i) {
            return i === index ? { key: entry.key, value: value } : entry;
          });
        });
      }, [setEnvEntries]);

      var removeEnvEntry = useCallback(function (index) {
        setEnvEntries(function (prev) {
          if (!prev) return prev;
      ...
      var addEnvEntry = useCallback(function () {
        setEnvEntries(function (prev) {
          if (!prev) return prev;
          var next = prev.slice();
          next.push({ key: '', value: '' });
      ...
      var saveWsEnv = useCallback(function () {
        if (wsPath === undefined || busy !== null || envEntries === null) return;
        setBusy('config');
        appendLog([{ time: nowTime(), repo: '-', message: '保存工作区 .env 配置', tone: 'info' }]);
        postJson('/git-config', { ws: wsPath, action: 'save-ws-env', entries: envEntries })
          .then(function (data) {
            appendLog([{ time: nowTime(), repo: '-', message: data.message, tone: data.ok ? 'ok' : 'fail' }]);
            if (data.ok && data.updated) {
              // Re-pull identity + credential views so the shortcut form stays fresh.
              return Promise.all([
                postJson('/git-config', { ws: wsPath, action: 'ws-cred' }),
                postJson('/git-config', { ws: wsPath, action: 'global-cred' }),
              ]).then(function (values) {
                var wsCred = values[0];
                var globalCred = values[1];
                var host = credScope === 'global'
                  ? (globalCred.host || wsCred.host || '')
                  : (wsCred.host || globalCred.host || '');
                var user = credScope === 'global'
                  ? (globalCred.username || wsCred.username || '')
                  : (wsCred.username || globalCred.username || '');
                setCredHost(host);
                setCredUser(user);
                setCredInfo(function (prev) {
                  return prev === null ? prev : {
                    remoteHosts: prev.remoteHosts,
                    envPath: data.envPath || prev.envPath,
                    envExists: true,
                    globalUsername: globalCred.username || '',
                    globalHasToken: globalCred.hasToken,
                    globalHost: globalCred.host || '',
                    globalFile: globalCred.file || prev.globalFile,
                  };
                });
              });
            }
          ...
          .then(function () { setBusy(null); });
      }, [wsPath, busy, envEntries, credScope, appendLog, setCredHost, setCredUser, setCredInfo, setBusy, setEnvEntries]);
      ...
      var tabConfig = h('div', { className: 'dshgb-form' },
        ...
        h('div', { className: 'dshgb-row' },
          h('label', { className: 'dshgb-check' },
            h('input', {
              type: 'radio',
              name: 'dshgb-cred-scope',
              checked: credScope === 'local',
              disabled: busyAny,
              onChange: function () { setCredScope('local'); setCfgLoaded(false); },
            }),
            '仓库级'),
          h('label', { className: 'dshgb-check' },
            h('input', {
        ...
        h('div', { className: 'dshgb-row' },
          h('input', {
            className: 'dshgb-input',
            placeholder: (credScope === 'global' ? '用户名（全局，所有仓库共用）' : '用户名（当前工作区）') + (credInfo && credInfo.globalUsername ? '，当前：' + credInfo.globalUsername : ''),
            value: credUser,
            disabled: busyAny,
        ...
        h('div', { className: 'dshgb-row' },
          ...
          h('span', { className: 'dshgb-hint', style: { flex: 1 } },
            ...
            (credScope === 'global' ? '；host + 账号全局共用' : '；host 按工作区配置，各仓库共用同一套账号')),
        ),
        h('div', { className: 'dshgb-row', style: { marginTop: '8px' } },
          h('span', { className: 'dshgb-form-label', style: { flex: 1 } },
            '工作区 .env 配置' + (credInfo && credInfo.envPath ? '（' + credInfo.envPath + '）' : '')),
          envEntries !== null ? h('button', {
            type: 'button', className: 'dshgb-btn dshgb-btn-small', disabled: busyAny,
            onClick: addEnvEntry,
          }, '添加键值') : null,
        ),
        h('div', { className: 'dshgb-hint' }, '展示并编辑当前工作区 .env 的键值；保存后写回 .env 文件（注释与其他内容保留）。'),
        envEntries === null
          ? h('div', { className: 'dshgb-hint' }, '（未加载）')
          : envEntries.length === 0
            ? h('div', { className: 'dshgb-hint' }, '（.env 为空或不存在）')
            : h('div', { className: 'dshgb-env-editor' },
                envEntries.map(function (entry, index) {
                  return h('div', { key: index, className: 'dshgb-env-row' },
                    h('input', {
                      className: 'dshgb-input dshgb-env-key',
                      placeholder: '键名',
                      value: entry.key,
                      disabled: busyAny,
                      onChange: function (e) { updateEnvEntry(index, e.target.value); },
                    }),
                    h('input', {
                      className: 'dshgb-input dshgb-env-value',
                      placeholder: '值',
                      value: entry.value,
                      disabled: busyAny,
                      onChange: function (e) { updateEnvEntry(index, e.target.value); },
                    ...
                    h('button', {
                      ...
                      title: '删除',
                      disabled: busyAny,
                      onClick: function () { removeEnvEntry(index); },
                    }, closeIcon(h)),
                  );
                })),
        envEntries !== null ? h('button', {
          type: 'button', className: 'dshgb-btn primary', disabled: busyAny,
          onClick: saveWsEnv,
        }, busy === 'config' ? spinnerIcon(h) : null, '保存 .env 配置') : null,
      );
      ...
      // Tab content scrolls independently; the log box is pinned to the bottom
      // and always visible (placeholder text when empty) so operation results
      // never get pushed out of view.
      var railBody = h('div', { className: 'dshgb-rail-body' },
        ...
        h('div', { className: 'dshgb-tab-content' }, tabBody),
        h('div', { className: 'dshgb-log', ref: logRef },
          log.length === 0
            ? h('div', { className: 'dshgb-log-empty' }, '暂无操作日志')
