    ...
    return function GitBranchDock(props) {
      ...
      var loadConfig = useCallback(function () {
        ...
        Promise.all([
          ...
          // username from the workspace .env / global, token left blank so the
          // user never re-types it unless they want to rotate it.
          setCredHost(wsCred.host || (wsCred.remoteHosts && wsCred.remoteHosts[0]) || '');
          setCredUser(wsCred.username || globalCred.username || '');
          setCredPass('');
          setCredInfo({
            remoteHosts: wsCred.remoteHosts || [],
            envPath: wsCred.envPath,
            envExists: wsCred.envExists,
            wsHasToken: wsCred.hasToken,
            ...
            if (data.ok) {
              ...
              setCredInfo(function (prev) {
                return prev === null ? prev : {
                  ...
                  envExists: true,
                  wsHasToken: password !== '' || prev.wsHasToken,
      ...
      var tabConfig = h('div', { className: 'dshgb-form' },
        ...
        h('div', { className: 'dshgb-row' },
          ...
          h('span', { className: 'dshgb-hint', style: { flex: 1 } },
            (credInfo && credInfo.wsHasToken ? '工作区 .env 已有 token' : '当前工作区 .env 无 token') +
            '；账号密码全局共用，各工作区只需配 host'),
        ),
      );
