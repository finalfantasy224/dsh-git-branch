...
(function () {
  ...
  function makeGitBranchDock(react, h) {
    ...
    return function GitBranchDock(props) {
      ...
      var credInfoInit = useState(null); // { remoteHosts, envPath, hasToken, globalUsername, globalHasToken, globalFile }
      ...
      var loadConfig = useCallback(function () {
        ...
        Promise.all([
          ...
          // Workspace-level defaults for the credential form. Fixed 仓库级
          // mode: host is derived from repo remote origins (shown as the
          // first detected host), username from the workspace .env, token is
          // global-only.
          var preferredHost = wsCred.host || (wsCred.remoteHosts && wsCred.remoteHosts[0]) || '';
          setCredHost(preferredHost);
          var preferredUser = wsCred.username || globalCred.username || '';
      ...
      var runCredential = useCallback(function () {
        ...
        setBusy('credential');
        appendLog([{ time: nowTime(), repo: '-', message: '保存凭证 → ' + host + '（当前工作区）', tone: 'info' }]);
        postJson('/git-config', {
          ws: wsPath,
          action: 'save-ws-cred',
          host: host,
          ...
          .then(function (data) {
            ...
            if (data.ok && data.updated) {
              ...
              return Promise.all([
                ...
                // Fixed 仓库级 mode: host from workspace view (remote-derived),
                // username from the workspace .env, falling back to global.
                var host = wsCred.host || (wsCred.remoteHosts && wsCred.remoteHosts[0]) || '';
                var user = wsCred.username || globalCred.username || '';
                setCredHost(host);
                setCredUser(user);
                setCredInfo(function (prev) {
                  return prev === null ? prev : {
      ...
      var tabConfig = h('div', { className: 'dshgb-form' },
        ...
        h('div', { className: 'dshgb-row' },
          h('input', {
            className: 'dshgb-input',
            placeholder: '用户名（当前工作区）' + (credInfo && credInfo.globalUsername ? '，当前：' + credInfo.globalUsername : ''),
            value: credUser,
            disabled: busyAny,
