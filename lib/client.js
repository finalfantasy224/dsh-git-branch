...
(function () {
  ...
  function makeGitBranchDock(react, h) {
    ...
    return function GitBranchDock(props) {
      ...
      // Identity (user.name / user.email) is always configured globally — git
      // commits use the global identity regardless of workspace.
      var cfgScope = 'global';
      ...
      // Credentials (host + username + password) all live in <ws>/.env.
      var credInfoInit = useState(null); // { remoteHosts, envPath, envExists, hasToken }
      ...
      var loadConfig = useCallback(function () {
        ...
        Promise.all([
          ...
          var wsCred = values[2];
          // All credentials live in the workspace .env: host + username (and
          // password, kept separate) are read from there on load.
          var preferredHost = wsCred.host || (wsCred.remoteHosts && wsCred.remoteHosts[0]) || '';
          setCredHost(preferredHost);
          setCredUser(wsCred.username || '');
          setCredPass('');
          setCredInfo({
            remoteHosts: wsCred.remoteHosts || [],
            envPath: wsCred.envPath,
            envExists: wsCred.envExists,
            hasToken: wsCred.hasToken,
          });
          // Full .env key/value list for the editor (null → not loaded yet).
          setEnvEntries(Array.isArray(values[3] && values[3].entries) ? values[3].entries : []);
          setCfgLoaded(true);
        }).catch(function () {
      ...
      }, [wsPath, cfgLoaded, appendLog, setBusy, setCfgName, setCfgEmail, setCfgLoaded, setCredHost, setCredUser, setCredPass, setCredInfo, setEnvEntries]);
      ...
      }, [wsPath, busy, cfgName, cfgEmail, appendLog, loadStatus]);
      ...
      var runCredential = useCallback(function () {
        ...
        setBusy('credential');
        appendLog([{ time: nowTime(), repo: '-', message: '保存凭证 → ' + host + '（写入工作区 .env）', tone: 'info' }]);
        postJson('/git-config', {
          ws: wsPath,
          action: 'save-ws-cred',
          host: host,
            ...
            if (data.ok) {
              ...
              setCredInfo(function (prev) {
                return prev === null ? prev : {
                  ...
                  envExists: true,
                  hasToken: data.envPath !== '' ? password !== '' || prev.hasToken : prev.hasToken,
                };
              });
            }
          })
        ...
        postJson('/git-config', { ws: wsPath, action: 'save-ws-env', entries: envEntries })
          .then(function (data) {
            ...
            if (data.ok && data.updated) {
              // Re-pull the credential view so the shortcut form stays fresh.
              return postJson('/git-config', { ws: wsPath, action: 'ws-cred' })
                .then(function (wsCred) {
                  var host = wsCred.host || (wsCred.remoteHosts && wsCred.remoteHosts[0]) || '';
                  setCredHost(host);
                  setCredUser(wsCred.username || '');
                  setCredInfo(function (prev) {
                    return prev === null ? prev : {
          ...
          .then(function () { setBusy(null); });
      }, [wsPath, busy, envEntries, appendLog, setCredHost, setCredUser, setCredInfo, setBusy, setEnvEntries]);
      ...
      var tabConfig = h('div', { className: 'dshgb-form' },
        onFocusConfig,
        h('div', { className: 'dshgb-row' },
          h('span', { className: 'dshgb-form-label', style: { flex: 1 } }, '身份配置（全局生效）'),
          h('button', {
            type: 'button',
        ...
        h('div', { className: 'dshgb-row' },
          h('span', { className: 'dshgb-form-label', style: { flex: 1 } }, '仓库凭证（host + 账号 + 密码）'),
        ),
        h('div', { className: 'dshgb-hint' },
          'host + 账号 + 密码全部存入当前工作区 .env（GIT_INSTANCE_URL / GIT_USERNAME / GIT_PASSWORD）。' +
          '子仓库 remote host：' +
        ...
        h('div', { className: 'dshgb-row' },
          ...
          h('input', {
            className: 'dshgb-input',
            type: 'password',
            placeholder: '密码 / token' + (credInfo && credInfo.hasToken ? '（已设置，留空保留）' : ''),
            value: credPass,
            disabled: busyAny,
        ...
        h('div', { className: 'dshgb-row' },
          ...
          h('span', { className: 'dshgb-hint', style: { flex: 1 } },
            (credInfo && credInfo.hasToken ? '工作区 .env 已存账号密码' : '工作区 .env 尚无账号密码') +
            '；全部写入当前工作区 .env'),
        ),
