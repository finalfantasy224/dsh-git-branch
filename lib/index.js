 ...
 * username/password. The workspace host lives in this .env too
 * (GIT_INSTANCE_URL) — it is written on save and read by the helper at
 * runtime, no remote-origin derivation needed.
 */
...
/** Stale host key in the global credential file — read for display only, scrubbed on write. */
const GLOBAL_HOST_KEY = 'GIT_INSTANCE_URL'
...
function readGlobalCredential() {
  ...
  // Prefer GIT_ keys, fall back to legacy GITLAB_ names. Host is stale/display-only.
  const host = values[GLOBAL_HOST_KEY] ?? values[LEGACY_INSTANCE_URL_KEY] ?? ''
...
/**
 * Persist the globally shared username/password. The file holds only the
 * plugin-managed keys (GIT_USERNAME / GIT_PASSWORD); a non-empty token
 * replaces the stored one, an empty token keeps the existing token so the
 * UI can leave the token blank to keep the current one. Hosts are never
 * stored globally (each repo's host is derived from its remote origin).
 * Legacy GITLAB_* keys are scrubbed and any stale GIT_INSTANCE_URL dropped.
 */
function writeGlobalCredential(username, token) {
  ...
  if (Object.prototype.hasOwnProperty.call(values, LEGACY_INSTANCE_URL_KEY)) delete values[LEGACY_INSTANCE_URL_KEY]
  if (Object.prototype.hasOwnProperty.call(values, GLOBAL_HOST_KEY)) delete values[GLOBAL_HOST_KEY]
  const dir = pluginDataDir()
  if (!existsSync(dir)) mkdirSync(dir, { recursive: true })
  let out = ''
  for (const key of [GLOBAL_USERNAME_KEY, GLOBAL_PASSWORD_KEY]) {
...
function writeGlobalHelper() {
  ...
  const script = [
    ...
    '# matches the host git asked about; username comes from that .env',
...
/**
 * Save the per-workspace credentials. Fixed 仓库级 mode (no global scope):
 *
 *   - host + username → <workspace>/.env (GIT_INSTANCE_URL / GIT_USERNAME)
 *   - token           → global credential file (GIT_PASSWORD, never in .env)
 *
 * The generated helper is registered so git resolves the workspace host from
 * .env and answers with the workspace username / global token; the user's
...
async function saveCredentials(wsPath, host, username, token) {
  ...
  const envResult = writeWorkspaceEnv(wsPath, host, username, '', {
    removeKeys: [],
  })
...
async function handleRoute(req, res) {
  ...
  if (route === '/git-config') {
    ...
    if (action === 'global-cred') {
      // Globally shared username/token (password only; no host stored —
      // each repo's host comes from its remote origin at runtime).
    ...
    if (action === 'save-ws-cred') {
      ...
      // Fixed 仓库级 mode: host + username → <ws>/.env, token → global
      // credential file; legacy GITLAB_* keys scrubbed, generated helper
      // registered so git answers with these credentials for the workspace.
      ...
      sendJson(res, 200, {
        ...
        envPath: saved.env,
        helper: saved.helper,
        message: `已保存：host + 账号 → ${saved.env}，密码 → ${saved.global}（helper: ${saved.helper}）`,
      })
      return
    }
