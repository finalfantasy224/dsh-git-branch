...
function writeWorkspaceEnv(wsPath, host, username, token, options = {}) {
    ...
    try {
      ...
      for (const line of text.split('\n')) {
        ...
        if (match && removeSet.has(match[1])) {
          // dropped
        } else if (match && Object.prototype.hasOwnProperty.call(updates, match[1]) && !removeSet.has(match[1])) {
  ...
  const updatedKeys = Object.keys(updates).filter((k) => !removeSet.has(k))
  for (const key of updatedKeys) {
    if (!seen.has(key)) out += `${key}=${updates[key]}\n`
  }
  writeFileSync(envPath, out, 'utf8')
  return { envPath, updated: updatedKeys, removed: Array.from(removeSet) }
}
