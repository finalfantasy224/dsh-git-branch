 ...
 * existing token untouched. Pass `removeKeys` to delete keys instead.
 */
function writeWorkspaceEnv(wsPath, host, username, token, options = {}) {
  ...
  if (removeKeys.length === 0) {
    ...
    updates[INSTANCE_URL_KEY] = `${scheme}${cleanHost}`
    updates[USERNAME_KEY] = username
    const newToken = typeof token === 'string' && token.trim() !== '' ? token : null
    if (newToken !== null) updates[TOKEN_KEY] = newToken
  }
  const removeSet = new Set(removeKeys)
  // Rewrite the file line by line, keeping unrelated content and comments.
  let out = ''
  let seen = new Set()
  if (existsSync(envPath)) {
    try {
  ...
  for (const key of Object.keys(updates)) {
    if (!seen.has(key)) out += `${key}=${updates[key]}\n`
  }
 ...
 *   - username + token  → global credential file (shared defaults)
 *   - host + username + token → <workspace>/.env (so the existing .git-credential
 *     helper keeps working with zero changes)
 */
function saveCredentials(wsPath, host, username, token) {
  const globalResult = writeGlobalCredential(username, token)
  const envResult = writeWorkspaceEnv(wsPath, host, username, token)
  return { global: globalResult.path, env: envResult.envPath }
}
...
async function handleRoute(req, res) {
  ...
  if (route === '/git-config') {
    ...
    if (action === 'save-ws-cred') {
      ...
      // Shared username/token → global defaults; host + credentials → workspace .env.
      const global = writeGlobalCredential(username, token)
      const env = writeWorkspaceEnv(wsPath, host, username, token)
      sendJson(res, 200, {
        ws: wsPath,
        ok: true,
        globalFile: global.path,
        envPath: env.envPath,
        globalUpdated: global,
        updated: env.updated,
        message: `已保存：全局凭证 ${global.path}，工作区配置 ${env.envPath}`,
      })
      return
    }
