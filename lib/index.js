 ...
 * scheme is provider-neutral (GitLab / GitHub / …). All credentials live in
 * the workspace .env — host, username AND password — plus the business key
 * REQUIREMENT_ID. There is no global credential file anymore.
 */
const GIT_PREFIX = 'GIT_'
const INSTANCE_URL_KEY = 'GIT_INSTANCE_URL'
const USERNAME_KEY = 'GIT_USERNAME'
const PASSWORD_KEY = 'GIT_PASSWORD'
const REQUIREMENT_ID_KEY = 'REQUIREMENT_ID'
/** New-line keys the plugin writes / manages in the workspace .env. */
const WORKSPACE_ENV_KEYS = [INSTANCE_URL_KEY, USERNAME_KEY, PASSWORD_KEY]
 ...
 * or empty — credentials (host/username/password), the requirement id and
 * SSH helpers, provider-neutral. Values are left blank.
 */
...
/** Compute the credentials view for a workspace: what the UI should show for host/username/password. */
function workspaceCredentialView(wsPath) {
  ...
  const password = env.values[PASSWORD_KEY] ?? env.values[LEGACY_TOKEN_KEY] ?? ''
  ...
  return {
    ...
    host: host ?? '',
    username,
    // Everything lives in the workspace .env now (no global credential file).
    hasToken: password !== '',
...
/**
 * Persist {host, username, password} into <workspace>/.env, preserving every
 * other line (the file may carry unrelated vars). Lines are rewritten in
 * place; missing keys are appended. Empty values leave the existing value
 * untouched (so the 配置 tab can leave the password blank to keep it).
 * Legacy GITLAB_* keys are scrubbed on write so the .env migrates to the
 * new GIT_ names.
 */
function writeWorkspaceEnv(wsPath, host, username, password, options = {}) {
  const { removeKeys = [] } = options
  const envPath = join(wsPath, '.env')
  const oldValues = {}
  if (existsSync(envPath)) {
  ...
  const entries = []
  if (host !== '') entries.push({ key: INSTANCE_URL_KEY, value: `${scheme}${cleanHost}` })
  if (username !== '') entries.push({ key: USERNAME_KEY, value: username })
  if (typeof password === 'string' && password !== '') entries.push({ key: PASSWORD_KEY, value: password })
...
/** Absolute path of the generated credential helper script. */
 ...
 * prints the workspace username/password (all stored in that .env).
 * Everything else is ignored so the helper is safe on unrelated hosts.
 */
function writeGlobalHelper() {
  ...
  const script = [
    '#!/bin/bash',
    '# Generated by dsh-git-branch — credential helper.',
    '# Walk up from the repo to find the workspace .env whose GIT_INSTANCE_URL',
    '# matches the host git asked about; username and password (GIT_USERNAME /',
    '# GIT_PASSWORD, legacy GITLAB_* names accepted) come from that .env.',
    ...
    '    fi',
    '    if [ -n "$url" ]; then',
    "      ih=$(printf '%s' \"$url\" | sed -E 's|https?://||' | sed -E 's|/.*$||' | sed -E 's|:[0-9]+$||')",
    '      if [ "$ih" = "$host" ]; then',
    "        username=$(grep '^GIT_USERNAME=' \"$dir/.env\" 2>/dev/null | head -1 | cut -d= -f2-)",
    '        if [ -z "$username" ]; then',
    "          username=$(grep '^GITLAB_USERNAME=' \"$dir/.env\" 2>/dev/null | head -1 | cut -d= -f2-)",
    '        fi',
    "        token=$(grep '^GIT_PASSWORD=' \"$dir/.env\" 2>/dev/null | head -1 | cut -d= -f2-)",
    '        if [ -z "$token" ]; then',
    "          token=$(grep '^GITLAB_TOKEN=' \"$dir/.env\" 2>/dev/null | head -1 | cut -d= -f2-)",
    '        fi',
...
/**
 * Save the workspace credentials. Everything lives in <workspace>/.env:
 *
 *   - host         → GIT_INSTANCE_URL
 *   - username     → GIT_USERNAME
 *   - password     → GIT_PASSWORD
 *   - requirementId → REQUIREMENT_ID (nullable → left untouched)
 *
 * There is no global credential file. The generated helper is registered so
 * git answers with these workspace credentials; the user's existing
 * .git-credential scripts keep working untouched.
 */
async function saveCredentials(wsPath, host, username, password, requirementId) {
  // Legacy GITLAB_* keys are scrubbed on write (writeWorkspaceEnv appends
  // LEGACY_ENV_KEYS to removeKeys).
  const envResult = writeWorkspaceEnv(wsPath, host, username, password, {
    removeKeys: [],
  })
  if (typeof requirementId === 'string' && requirementId !== '') {
    writeEnvEntries(wsPath, [{ key: REQUIREMENT_ID_KEY, value: requirementId }], { removeKeys: [] })
  }
  const helper = await registerGlobalHelper()
  return { env: envResult.envPath, helper: helper.helper }
}
  ...
  if (route === '/git-config') {
    ...
    if (action === 'ws-cred') {
      ...
      sendJson(res, 200, {
        ...
        requirementId: env.values[REQUIREMENT_ID_KEY] ?? '',
        remoteHosts: hosts,
    ...
    if (action === 'save-ws-cred') {
      const host = typeof body?.host === 'string' ? body.host.trim() : ''
      const username = typeof body?.username === 'string' ? body.username.trim() : ''
      const password = typeof body?.token === 'string' ? body.token : ''
      ...
      if (password.length > 4000 || password.includes('\n') || password.includes('\r') || password.includes('\0')) {
        sendJson(res, 400, { error: 'token 非法' })
        return
      }
      if (requirementId.length > 200 || requirementId.includes('\n') || requirementId.includes('\r') || requirementId.includes('\0')) {
        sendJson(res, 400, { error: 'REQUIREMENT_ID 非法' })
        return
      }
      // host + username + password (+ REQUIREMENT_ID) all go to <ws>/.env;
      // legacy GITLAB_* keys scrubbed, generated helper registered so git
      // answers with these workspace credentials. No global credential file.
      const saved = await saveCredentials(wsPath, host, username, password, requirementId)
      sendJson(res, 200, {
        ws: wsPath,
        ok: true,
        envPath: saved.env,
        helper: saved.helper,
        message: `已保存：host + 账号 + 密码${requirementId !== '' ? ' + REQUIREMENT_ID' : ''} → ${saved.env}（helper: ${saved.helper}）`,
      })
      return
    }
    ...
    if (action === 'set-kv' || action === 'unset-kv') {
      ...
      if (action === 'unset-kv') {
        const results = await mapPool(repos, CONCURRENCY, async (repo) => {
          const result = await git(repo.path, ['config', '--local', '--unset-all', key], TIMEOUT_SWITCH_MS)
          return { repo: repo.name, ok: result.code === 0, skipped: result.code !== 0, message: result.code === 0 ? '已删除' : firstLine(result.stderr) || '删除失败' }
        })
        sendJson(res, 200, { ws: wsPath, key, results })
        return
      }
      ...
      const results = await mapPool(repos, CONCURRENCY, async (repo) => {
        const result = await git(repo.path, ['config', '--local', key, value], TIMEOUT_SWITCH_MS)
...
/** Test hooks — exposed for sandbox tests only (no dsh runtime impact). */
