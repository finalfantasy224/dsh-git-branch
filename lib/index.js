...
/**
 * Persist the globally shared username/password. The file holds only the two
 * plugin-managed keys (GIT_USERNAME / GIT_PASSWORD); a non-empty token
 * replaces the stored one, an empty token keeps the existing one (x the
 * deleted line) so the UI can leave the token blank to keep the current one.
 * Legacy GITLAB_* keys are scrubbed on write.
 */
...
function writeGlobalHelper() {
  ...
  const script = [
    ...
    '# Looks up each workspace .env (GIT_INSTANCE_URL / legacy GITLAB_INSTANCE_URL)',
    '# for a host match, then answers with the globally stored username/password.',
    'set -euo pipefail',
    '',
    'action="${1:-}"',
 ...
 *   - host                           → <workspace>/.env (GIT_INSTANCE_URL)
...
async function saveCredentials(wsPath, host, username, token) {
  ...
  // Remove the per-workspace credential keys; writeWorkspaceEnv also migrates
  // legacy GITLAB_* names away. Keep everything else.
  const envResult = writeWorkspaceEnv(wsPath, host, '', '', {
    removeKeys: [USERNAME_KEY, LEGACY_USERNAME_KEY, LEGACY_TOKEN_KEY],
  })
