 ...
 *
 * (REQUIREMENT_ID is a plain business key the user edits in the .env editor.)
 ...
 */
async function saveCredentials(wsPath, host, username, password) {
...
async function handleRoute(req, res) {
  ...
  if (route === '/git-config') {
    ...
    if (action === 'save-ws-cred') {
      ...
      // host + username + password all go to <ws>/.env; legacy GITLAB_*
      // keys scrubbed, generated helper registered so git answers with
      // these workspace credentials. No global credential file anymore.
      const saved = await saveCredentials(wsPath, host, username, password)
      sendJson(res, 200, {
        ws: wsPath,
        ok: true,
        envPath: saved.env,
        helper: saved.helper,
        message: `已保存：host + 账号 + 密码 → ${saved.env}（helper: ${saved.helper}）`,
      })
      return
    }
